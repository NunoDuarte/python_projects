...
def launcher():
    ...
    ipc_pub_url = xsub_socket.last_endpoint.decode('utf8').replace("0.0.0.0","10.0.21.96")

    xpub_socket = zmq_ctx.socket(zmq.XPUB)
    xpub_socket.bind(ipc_sub_url)
    ipc_sub_url = xpub_socket.last_endpoint.decode('utf8').replace("0.0.0.0","10.0.21.96") #127.0.0.1

    pull_socket = zmq_ctx.socket(zmq.PULL)
    pull_socket.bind(ipc_push_url)
    ipc_push_url = pull_socket.last_endpoint.decode('utf8').replace("0.0.0.0","10.0.21.96")
